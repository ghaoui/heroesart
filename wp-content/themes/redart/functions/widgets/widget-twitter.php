			...
			if($username && $consumer_key && $consumer_secret && $access_token && $access_token_secret && $count) { 
				...
				if($twitter && is_array($twitter)) {
					foreach( $twitter as $tweet ):
						...
						$latestTweet = preg_replace('/http'.redart_ssl().':\/\/([a-z0-9_\.\-\+\&\!\#\~\/\,]+)/i', '<a href="http'.redart_ssl().'://$1" target="_blank">http://$1</a>', $latestTweet);
						$latestTweet = preg_replace('/@([a-z0-9_]+)/i', '<a href="http'.redart_ssl().'://twitter.com/$1" target="_blank">@$1</a>', $latestTweet);
